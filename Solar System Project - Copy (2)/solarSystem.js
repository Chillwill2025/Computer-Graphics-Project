...
var planetBTrail = [];
var planetCTrail = [];
var maxTrailLength = 500; // 3/5 of 1250
...
function drawOrbit(body){
    if(body.orbitRadius > 0.001){
        ...
        for(var i=0;i<segments;i++){
            ...
            // Place orbit ring: for children, draw flat in parent space; for roots with inclination
            if(body.parent){
                // Child bodies: orbit ring is flat in parent's local space, just transform by parent's world matrix
                var parentWorld = body.parent.worldMatrix();
                var p2 = mult(parentWorld, p);
                pts.push(p2);
            } else {
                // Root bodies: apply own inclination and translate to current position
    ...
    function drawTrail(trail, color){
        if(trail.length > 1){
            ...
            for(var start = 0; start < trail.length - 1; start += segmentSize){
                ...
                var alpha = 1.0;
                if(age > 500){
                    var fadeRange = age - 500;
                    var maxFade = trail.length - 500;
